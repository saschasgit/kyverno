...
Der Grund ist, dass sich dalle Reports nicht auf die Art der Policy (Policy <-> ClusterPolicy) beziehen, sondern darauf, ob das zu prüfende Objekt ein Namespacegebundenes Objekt oder ein Clusterobjekt ist. Wir müssen daher in die "BackgroundScanReports" schauen:
...
Es gibt Policies, in welchen man sich auf einen bestimmten JSON-Pfad beziehen muss. In solchen Policies sieht man dann z.B. einen Pfad wie "request.object" oder "request.namespace". Diese Pfade findet man jedoch in Ressourcen wie einem Namespace nicht. Der Gund ist, dass am Admission Controller nicht das Objekt "Namespace" ankommt, sondern ein "AdmissionRequest" für ein Namespace-Objekt.
